...
def send_email(
    ...
    if settings_obj.resend_api_key:
        try:
            from_email = "SafetyPaws Reports <safetypaws@safetypaws.developmentnous.nz>"
            send_resend_email(
...
async def serve_qr_code():
    ...
    qr_html = ROOT_DIR / "assets" / "qr_page.html"
    if qr_html.exists():
        return HTMLResponse(content=qr_html.read_text())
